...
echo "  WebUI: https://${FORTIGATE_IP}/ui      (8.0.1 moved the GUI to /ui)"
echo "         login: admin / <password>  (no vendor default — set via DB on first deploy)"
echo ""
...
echo "       Path: /v1/chat/completions   (OpenAI schema only)"
echo "       Route to Guard from step B → Deploy"
echo ""
echo "  D) Virtual Keys → Create a key (Bearer token for /v1/chat/completions)"
echo ""
echo "Verify models: kubectl get isvc -n aegis-mesh && kubectl get pods -n aegis-mesh"
echo "Test traffic:  FORTIAIGATE_URL=https://${FORTIGATE_IP} FORTIAIGATE_API_KEY=sk-... \\"
echo "                 ./tests/fortiaigate-8.0.1/run-security-tests.sh --insecure"
